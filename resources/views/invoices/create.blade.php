...
<div class="content-card">
    <div class="form-header">
        <h1 class="form-title">Create New Invoices</h1>
        <div class="form-actions">
            <a href="{{ route('help.show', ['form' => 'invoices.create']) }}" target="_blank" class="btn btn-secondary" title="Help" style="background: #757575; color: white; padding: 8px 12px; font-size: 16px; font-weight: bold;">?</a>
        </div>
            ...
            <div class="form-group">
                ...
                <select name="quotation_id" class="form-control">
                    ...
                    @foreach($quotations as $quotation)
                        <option value="{{ $quotation->id }}" {{ old('quotation_id', $preSelected['quotation_id'] ?? '') == $quotation->id ? 'selected' : '' }}>{{ $quotation->quotation_number }}</option>
                    @endforeach
                </select>
                @error('quotation_id')
                    <div class="error">{{ $message }}</div>
