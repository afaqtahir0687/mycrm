...
<div class="content-card">
    <div class="form-header">
        <h1 class="form-title">Create New Support Tickets</h1>
        <div class="form-actions">
            <a href="{{ route('help.show', ['form' => 'support-tickets.create']) }}" target="_blank" class="btn btn-secondary" title="Help" style="background: #757575; color: white; padding: 8px 12px; font-size: 16px; font-weight: bold;">?</a>
        </div>
    ...
    <form method="POST" action="{{ route('support-tickets.store') }}">
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 20px;">
            ...
            <div class="form-group">
                ...
                <input type="number" name="sla_hours" value="{{ old('sla_hours') }}" >
                @error('sla_hours')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div style="display: flex; gap: 10px;">
            <button type="submit" class="btn btn-primary">Create Support Tickets</button>
